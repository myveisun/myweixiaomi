# Weixiaomi — 飞牛 FnOS 安装包

本目录包含将 [myveisun/myweixiaomi](https://github.com/myveisun/myweixiaomi) 打包为飞牛 fnOS `.fpk` 安装包所需的全部文件，并附带自动同步上游 + 自动构建的 GitHub Actions。
...
| 版本 | 包名 | 体积 | 运行方式 | 依赖 |
|------|------|------|----------|------|
| **Docker 版** | `Weixiaomi-fnos-docker-<ver>.fpk` | ~8 KB | 飞牛自动从 GHCR 拉取 `ghcr.io/myveisun/weixiaomi-commercial:latest` 镜像运行 | 宿主需有 Docker 运行时 |
| **本地版（非 Docker）** | `Weixiaomi-fnos-native-<ver>.fpk` | ~560 MB | 自带 Python 3.12 运行时 + 前端 + 核心依赖，原生运行在飞牛主机 | 无需 Docker |
...
> 两款包随正式版一起挂在 **`v*` GitHub Release** 上：`Weixiaomi-fnos-docker-<ver>.fpk` / `Weixiaomi-fnos-native-<ver>.fpk`。
...
│   ├── config/
│   │   ├── privilege       # 权限声明（docker-weixiaomi 用户）
│   │   └── resource        # 资源声明（docker-project + 数据共享目录）
│   ├── wizard/
│   │   └── install         # 安装向导（可配置管理员账号/密码、日志级别、LLM 密钥）
│   ├── app/
│   │   ├── docker/
│   │   │   └── docker-compose.yaml   # 引用 ghcr.io/myveisun/weixiaomi-commercial:latest
│   │   └── ui/
│   │       ├── config                # 桌面图标入口
...
2. **镜像复用**：不再重新 build 镜像；`ensure-image` 轮询等待 `ghcr.io/myveisun/weixiaomi-commercial:{version}`（由 docker-publish 推送）。Docker 版 `.fpk` 仅打包 compose，运行时拉取该镜像。
3. **Wheel 复用**：Native 版优先从同版本 GitHub Release（`v*`）下载 `octop-*.whl`；若缺失再回退源码构建前端 + wheel。
4. **安装包构建**：`fpk` / `native` job 用 `scripts/build-fpk.sh` 打包，产物 `Weixiaomi-fnos-docker-<ver>.fpk` / `Weixiaomi-fnos-native-<ver>.fpk` 挂到同一个 `v*` GitHub Release（与 wheel、桌面包并列）。

## 本地构建 .fpk（无需 Docker）

```bash
bash scripts/build-fpk.sh            # 仅 Docker 版  → dist/Weixiaomi-fnos-docker-<version>.fpk
bash scripts/build-fpk.sh docker     # 仅 Docker 版
bash scripts/build-fpk.sh native     # 仅本地版      → dist/Weixiaomi-fnos-native-<version>.fpk
```
...
1. 飞牛「应用中心 → 设置 → 手动安装应用」选择对应 `.fpk`：
   - 想用 Docker 跑、主机已装 Docker → 选 `Weixiaomi-fnos-docker-<version>.fpk`
   - 不想依赖 Docker、希望自带运行时原生运行 → 选 `Weixiaomi-fnos-native-<version>.fpk`
2. 安装向导中设置管理员账号/密码、日志级别、LLM 密钥（可选）。
3. 安装完成后桌面出现「威小蜜AI」图标，浏览器打开 `http://<设备IP>:8088`。
4. Docker 版镜像首次会从 GHCR `ghcr.io/myveisun/weixiaomi-commercial:latest` 拉取；请确保该包为 Public（首次推送后可在 GitHub Packages 设置）。本地版无需联网拉镜像。Playwright Chromium 不预装，需要远程浏览器时在控制台按需安装。
