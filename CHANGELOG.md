...
- 修复桌面安装版（`.exe` / `.dmg`）与便携版（`.zip`）中的应用图标仍为上游 Octop 图标的问题：应用窗口图标、系统托盘图标、安装程序图标以及 FnOS 安装包图标全部替换为威小蜜AI（VS Agent）品牌图标

### 变更

- 品牌收尾统一为 Weixiaomi：FnOS 安装包说明文档、Docker 发布工作流注释、Docker Hub 镜像名与 FnOS 生命周期日志前缀
- `pyproject.toml` 作者信息更新为 `VeiSun Networks`

## [1.2.0] - 2026-09-14

### 新增

- 桌面版与便携版形态区分：桌面安装版提供专属下载按钮与提示文案，更新/下载新版安装包指向桌面 exe 资产
- 数据目录随安装路径存放：优先级 `OCTOP_HOME` 环境变量 → exe 所在目录下的 `user-data` 子目录（可写时）→ 回退 `~/.octop`；安装于仅读目录时自动迁移旧 `~/.octop` 用户数据

### 变更

- 品牌全面统一为 Weixiaomi（威小蜜AI）：构建元数据、便携包/桌面包命名、发布下载链接与 GitHub 仓库对齐为 `Weixiaomi-*` / `myveisun/myweixiaomi`
