...
var desktopCopy = map[Locale]map[string]string{
	LocaleEN: {
		copyStatusConnecting:       "Connecting to VS Agent…",
		copyStatusCheckingRuntime:  "Checking the runtime…",
		copyStatusStartingService:  "Starting the VS Agent service…",
		copyStatusReady:            "VS Agent is ready",
		...
		copyHealthNotReady5xx:      "VS Agent did not become ready within %s (%s). The service responded but is not ready yet. Try again, or check the terminal logs.",
		copyHealthNotReadyConnect:  "VS Agent did not become ready within %s (%s). Could not connect — make sure VS Agent is running.",
		copyHealthNotReady:         "VS Agent did not become ready within %s (%s). Make sure VS Agent is running at this address, or check the terminal logs.",
	},
	LocaleZH: {
		copyStatusConnecting:       "正在连接威小蜜AI…",
		copyStatusCheckingRuntime:  "正在检查运行环境…",
		copyStatusStartingService:  "正在启动威小蜜AI 服务…",
		copyStatusReady:            "威小蜜AI 已就绪",
		...
		copyHealthNotReady5xx:      "威小蜜AI 服务未在%s内就绪（%s）。服务已响应但尚未就绪，请稍后再试，或查看终端日志。",
		copyHealthNotReadyConnect:  "威小蜜AI 服务未在%s内就绪（%s）。目前无法连接该地址，请确认威小蜜AI 正在运行。",
		copyHealthNotReady:         "威小蜜AI 服务未在%s内就绪（%s）。请确认本机已启动威小蜜AI，且地址、端口正确；也可查看终端日志。",
	},
}
